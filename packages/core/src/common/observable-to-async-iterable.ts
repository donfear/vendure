...
export function observableToAsyncIterable<T>(source: Observable<T>): AsyncIterableIterator<T> {
    ...
    // Assigned once `subscribe()` returns, which a synchronously completing source precedes.
    let subscription: Subscription | undefined;

    function fail(err: Error) {
        error = err;
        queue.length = 0;
        const consumers = waiting.splice(0, waiting.length);
        stop();
        if (consumers.length) {
            error = undefined;
            consumers.forEach(c => c.reject(err));
        }
    }
    ...
    return {
        ...
        // Ending the iteration from the consumer side discards what it has not consumed.
        return: () => {
            queue.length = 0;
