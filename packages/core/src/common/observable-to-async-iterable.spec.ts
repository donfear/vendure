...
describe('observableToAsyncIterable()', () => {
    ...
    it('handles a source which completes synchronously', async () => {
        const iterator = observableToAsyncIterable(of(1, 2));
    ...
    it('buffers undefined as a value like any other', async () => {
        ...
        expect(await iterator.next()).toEqual({ value: 'after', done: false });
    });

    it('resolves a waiting consumer once the iteration is returned', async () => {
        const source = new Subject<number>();
        const iterator = observableToAsyncIterable(source);
        const next = iterator.next();
    ...
    it('rejects every waiting consumer when the source errors', async () => {
        const source = new Subject<number>();
        const iterator = observableToAsyncIterable(source);
