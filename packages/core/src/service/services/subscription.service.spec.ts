...
describe('SubscriptionService', () => {
    ...
    let invalidatedSessionTokens: Set<string>;

    beforeEach(() => {
        eventStream = new Subject<VendureEvent>();
        const eventBus = {
            ofType: vi.fn((type: any) => ({
        ...
        invalidatedSessionTokens = new Set();
        const sessionService = {
            getSessionFromToken: vi.fn(async (token: string) =>
                invalidatedSessionTokens.has(token) ? undefined : { token },
            ),
    ...
    it('completes the subscription once the session has been invalidated', async () => {
        const ctx = createCtx(1);
        (ctx as any)._session = { id: '1', token: 'logged-out', expires: new Date(Date.now() + 100_000) };
        const iterator = service.fromEvent(ctx, 'testField', TestEvent, event => ({
            message: event.message,
        }));
        ...
        eventStream.next(new TestEvent(createCtx(1), 'before logout'));
        await tick();
        invalidatedSessionTokens.add(ctx.session?.token as string);
        eventStream.next(new TestEvent(createCtx(1), 'after logout'));
        await tick();

        expect(values).toEqual([{ testField: { message: 'before logout' } }]);
    ...
    describe('publish() / subscribe() over the relay strategy', () => {
        ...
        it('delivers payloads from any Channel when restrictToChannel is false', async () => {
            ...
            await service.publish(superadminCtx(), 'testKey', { message: 'other channel' });
            await tick();
    ...
    it('rejects a payload which would not survive being relayed', async () => {
        const ctx = createCtx(1);
