...
import { collectSubscription, triggerUntilReceived } from './utils/collect-subscription';
...
function tryConnect(headers: Record<string, string>): Promise<number | string> {
    ...
    return new Promise(settle => {
        ...
        const resolve = (outcome: number | string) => {
...
describe('GraphQL subscriptions with cookie-based auth', () => {
    ...
    it('authenticates a subscription with the session cookie set over http', async () => {
        ...
        await triggerUntilReceived(
            () => adminClient.query(TRIGGER_TEST_EVENT, { message: 'via-cookie', productId: 'T_1' }),
    ...
    it('treats a cookie with a broken signature as anonymous, even when it carries a valid token', async () => {
        // the very cookie which the first test authenticated with, minus its signature
        const tamperedCookie = adminClient
            .getCookieHeader()
            .replace(/session\.sig=[^;]+/, 'session.sig=forged');
        const subscription = collectSubscription(
            adminClient.subscribe(TEST_EVENT_SUBSCRIPTION, undefined, {
                connectionParams: null,
