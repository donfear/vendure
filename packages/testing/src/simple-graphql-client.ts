...
export class SimpleGraphQLClient {
    ...
    setAuthToken(token: string) {
        ...
        // a stored session cookie would otherwise take precedence over the token
        this.cookies.clear();
    }
    ...
    setChannelToken(token: string | null) {
        ...
        const { channelTokenKey } = this.vendureConfig.apiOptions;
        if (channelTokenKey) {
            if (token === null) {
                // `null` would be sent as a literal "null" header, i.e. a token no Channel has
                delete this.headers[channelTokenKey];
            } else {
                this.headers[channelTokenKey] = token;
            }
        }
    }
    ...
    async fetch(url: string, options: RequestInit = {}): Promise<Response> {
        ...
        const cookieHeader = this.usesCookies() ? this.getCookieHeader() : '';
        const headers = {
    ...
    private usesCookies(): boolean {
        const { tokenMethod } = this.vendureConfig.authOptions ?? {};
        return tokenMethod === 'cookie' || (Array.isArray(tokenMethod) && tokenMethod.includes('cookie'));
    }

    /** A minimal cookie jar: enough for a session cookie to be set, replaced and cleared. */
    private storeCookies(response: Response) {
        ...
        for (const cookie of setCookies) {
            ...
            const cleared = attributes.some(attribute => {
                const [key, attributeValue] = attribute.split('=');
                return (
                    (key.toLowerCase() === 'max-age' && Number(attributeValue) <= 0) ||
                    (key.toLowerCase() === 'expires' && new Date(attributeValue) < new Date())
                );
            });
            if (cleared || value === '') {
